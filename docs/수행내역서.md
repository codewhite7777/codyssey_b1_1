...
확인 포인트:
- `Port 20022` (sshd_config) + `port 20022` (sshd -T 효과적 설정) → 양쪽 일치
- `PermitRootLogin no` (sshd_config) + `permitrootlogin no` (sshd -T) → root 차단 적용
- `LISTEN 0.0.0.0:20022` + `[::]:20022` → IPv4/IPv6 모두 20022 에서 수신 (22 부재)
...
확인 포인트:
- `agent-admin`: primary=agent-admin, 보조=agent-common, agent-core ✅
- `agent-dev`: primary=agent-dev, 보조=agent-common, agent-core ✅
- `agent-test`: primary=agent-test, 보조=agent-common (agent-core 미포함 — 명세대로 차단)
- `getent group agent-common` → agent-admin, agent-dev, agent-test 세 명 모두 포함
- `getent group agent-core` → agent-admin, agent-dev 만 포함 (agent-test 부재 → 키 접근 차단의 그룹 측 기반)
