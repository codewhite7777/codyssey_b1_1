...
![log-rotate.sh --dry-run 정상 동작](../evidence/11-log-rotate-dryrun.png)

확인 포인트:
- `[INFO] DRY RUN 모드 — 실제 변경 없음` — 안전 검증 (실제 압축·삭제 안 함)
- `[INFO] DRY: mkdir -p /var/log/monitor/agent-app/archive` — archive 디렉토리 미존재 시 자동 생성 의도 (실제 실행 시 즉시 mkdir, dry-run 이라 안내만)
- `[1/2] 7일+ 경과 ... 대상 파일 0개 — 압축 단계 skip` — 명세 *"대상 파일 0개 안전 처리"* 충족
- `[2/2] 30일+ 경과 ... 대상 파일 0개 — 삭제 단계 skip` — 동일 안전 처리
- `종합 결과: 압축 0, 삭제 0, 경고 0, 에러 0` — 등급별 카운트 정상

dry-run 통과 = 스크립트의 *syntax·로직·예외 처리* 가 모두 정상이라는 증거. 매일 03:00 cron.d 실행 시 실제 7일+ 경과 로그가 있으면 자동 압축·이동·삭제.
...
### 증거 파일
- `evidence/verify-all-pass.png` — **47/47 PASS** 한 화면 (10 카테고리 모두)
- `evidence/11-log-rotate-dryrun.png` — 보너스 2 dry-run 정상 동작
